...
const tabs: Tab[] = [
  {
    ...
    bullets: ['Recomendação ML com XGBoost', 'Análise profunda com Gemini', 'Ajuste automático por aging', 'Gestão de vendas nos portais'],
  },
  {
    id: 'acquisitions',
    label: 'Aquisições',
    ...
    bullets: ['Scout varre 3 plataformas', 'Analyst calcula deal scores', 'Valuator avalia com IA', 'Recommender sugere compras'],
  },
  {
    id: 'market',
    ...
    bullets: ['Scraping multi-plataforma', 'Progresso em tempo real', 'Histórico de pesquisas', 'Filtros avançados'],
  },
  {
    id: 'crm',
    label: 'CRM',
    ...
    bullets: ['Kanban com drag & drop', 'Lead scoring inteligente', 'WhatsApp integrado', 'Relatórios automáticos'],
  },
];
...
export default function FeatureShowcase() {
  ...
  return (
    <div ref={sectionRef} style={{ height: `${tabs.length * 100}vh` }} className="relative">
      {/* Sticky container */}
      <div className="sticky top-0 h-screen flex items-center bg-navy-dark/30 overflow-hidden">
            ...
            {tabs.map((tab, i) => (
              <button
                ...
                onClick={() => {
                  ...
                  const top = sectionRef.current.offsetTop + (i / tabs.length) * sectionRef.current.offsetHeight;
                  window.scrollTo({ top, behavior: 'smooth' });
                }}
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                  i === activeIdx ? 'bg-orange scale-125' : 'bg-white/20 hover:bg-white/40'
                }`}
              ...
              <div className="perspective-1000 relative order-2 lg:order-1">
                ...
                <div className="relative rounded-xl border border-white/10 bg-navy-dark shadow-xl overflow-hidden tilt-3d">
                  ...
                  <div className="aspect-[16/10] bg-gradient-to-br from-navy to-navy-dark p-4 overflow-hidden">
                    {active.id === 'pricing' && <PricingMock />}
                    {active.id === 'acquisitions' && <AcquisitionsMock />}
                    {active.id === 'market' && <MarketMock />}
                    {active.id === 'crm' && <CRMMock />}
                  </div>
                </div>
              </div>
